...
We know that the username is `john`, we need to Brute Force the password.
...
```
John is quite a literal minded person. He literally just used `john` as his username!! 
He said he tried to jazz up his password, but knowing him, it's probably the most obvious choice...
```

I opted for a small Wordlist to speed up the process, instead of the classic `rockyou.txt`, if no password found, i would opt for `rockyou.txt`, and otherwise it will be time saving.
...
Login and take the flag which shown as an Alert Popup! 
...
From here, we can start some code analyse.
...
Once downloaded, give it execution right using `chmod +x` and run it to see how it work.
...
> Note : I used `.`, because the `.git` folder is in the same location of the script.
...
Finaly, read all the files using `cat` and search for the string `texsaw{*` recursively.
...
Once downloaded we extract it's content using the password `infected` credit to <a href="https://vx-underground.org/">VX-Underground</a> for the password :D.
...
We decode the base64 string using `bash`, `base64` and `gzip` as the decoded base64 is compressed.
