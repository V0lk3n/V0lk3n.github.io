...
One of the best CTF event i played, and will deffinitvely be there at the 2025 edition!
...
Here we can see that 73,2% of the packets are request on the QUIC protocol which is a network protocol based on UDP, we see also 18.8% of packets using TCP where 6 packets use HTTP.

HTTP can be really interesting in capture analyse, and as there is only 6 packet, let's look at it in first. 
...
We can see two interesting request, a GET request to `/freediscordnitro`and a POST request to `/rj1893rj1joijdkajwda`, both request are made from `192.168.116.138` to `192.168.116.135`.

I right click on them and follow the HTTP stream. Let's start by the GET request.
...
Ive cut the base64 using `...` to get a shorter output for the writeup. As we can see, it seem to be an obfuscated powershell script, we know this because of the variable `$variablename` and because of this line `"Inv"+"OKe"+"-EX"+"pRe"+"SSI"+"On" ; New-alIaS -Name pWn -VaLuE $U9COA51JG8eTcHhs0YFxrQ3j -FoRcE ; pWn $lOADcODEoPX3ZoUgP2T6cvl3KEK ;` which clearly seem to be Powershell command.

Next let's take a look of the POST request, i right click on it and follow the HTTP stream too (you can use the shortcut CTRL+ALT+SHIF+H too once selected).
...
Here we can see that the POST request seem send a file called `rj1893rj1joijdkajwda` to a python server hosted by http.server python module. 
...
Now i got the powershell script `freediscordnitro` from the GET request, and the `rj1893rj1joijdkajwda` file sent with the POST request.

With this, my next goal was to deobfuscate the powershell script.

To do this, i used a powershell tools that i wasn't able to run on my linux powershell. So i run my windows virtual machine for this work.

Once my vm started, i download PowerDecode tool from <a href="https://github.com/Malandrone/">Malandrone</a>, you can find it bellow.

<a href="https://github.com/Malandrone/PowerDecode">PowerDecode</a>.

Once downloaded on my VM, i copy the powershell script in the same directory into my VM, and run the tool.
...
This open a windows to pick our the powershell script that we want to deobfuscate, then another windows to pick the output folder where we want the result to be saved. Also, it said that the powershell script need to be `.ps1 or .txt`, so i renamed `freediscordnitro` to `freediscordintro.ps1`.

Once the `freediscordnitro.ps1` powershell script and the output folder chosen. The tool start to deobfuscate.
...
We see the wole deobfuscating process and at the end it said that the report is saved to the chosen output folder.

Let's take a look to the report. Scrolling a bit, i can find the plain text powershell script :
...
Also the tool run some interesting analyse.
...
[ 0 ] Cannot connect to:  http://192.168.116.135:8080/rj1893rj1joijdkajwda

[404: Not Found] -  https://discord.com/api/v9/users/
...
Script attempted to print on screen the following text:  
______              ______ _                       _   _   _ _ _               _____  _____  _____   ___ 
|  ___|             |  _  (_)                     | | | \ | (_) |             / __  \|  _  |/ __  \ /   |
| |_ _ __ ___  ___  | | | |_ ___  ___ ___  _ __ __| | |  \| |_| |_ _ __ ___   ' / /'| |/' |' / /'/ /| |
|  _| '__/ _ \/ _ \ | | | | / __|/ __/ _ \| '__/ _ | | .  | | __| '__/ _ \    / /  |  /| |  / / / /_| |
...
Script attempted to print on screen the following text:  Generating Discord nitro keys! Please be patient...
```

Great. Now we need to analyse the powershell script.

First, it set the URL where to send the data, this show us that the file `rj1893rj1joijdkajwda` containt the stolen data, and there is a `steal` function which seem to steal a token.
...
Then a fake `GenerateDiscordNitroCodes` function, to trick the user who don't know that this is a malware.
...
Then a `Encrypt-String` function that encrypt the stolen data using the AES encryption and it's key, to base64 strings.
...
Oh, we got the first part of the flag!
...
Then it use all the token found and use the `GetDiscord-UserInfo` function to retrieve ID, Email, GlobalName and Token of the infected user.
...
Then we found some other variabls, the AES Key encoded as base64,  the user infos are converted to Json format, and the data are encrypted using the AES key. And finally it send the POST request containing the stolen data to the attacker.
...
The script end with a fake Discord Nitro keys, to trick the target. Explaining the challenge name FakeBoost because of the Discord Nitro Boost.
...
Now, we need to decode the stolen data using the AES key. We already extracted them from the capture file, its the `rj1893rj1joijdkajwda` file as discovered into the powershell script. 

To achieve this, i know two solutions. The first is to make a script which will decode the data. The second is to use AES Encryption Decryption Online tool, but i really dont like this way. Anyway bellow is the website. Basically, we can use openssl too, i was going for this way, but i failed all my attempt... I guess i need to find an IV value to use it, or maybe exploit it like a key derivation or something else, but my coding skills and code review skills are limited to achieve this.

<a href="https://www.devglan.com/online-tools/aes-encryption-decryption">AES Encryption Decryption Online Tool</a>

As i really don't like take unjustified honor, i admit that i'm horribly bad at coding, and to create the decoder, i needed to user ChatGPT. I sent to him the powershell script and ask him to create the decryption script.
...
Great, finally we decode the Email which is as base64.
...
Running wireshark, and looking at Protocol Hiearchy under `Statistics > Protocol Hierarchy`, show that 3,9% of packets use UDP protocol where 3,7% use DNS protocol. 95.6% use TCP protocol where 8.6% use SMTP protocol.
...
SMTP protocol seem interesting, so i filtred the capture packet using the `smtp` filter.
...
We can see that the mail said `Attached is a part of the file. Password: S3W8yzixNoL8` and that the mail name is `Secure File Transfer`. Also we see that the attachment is a zip called `efcfd.zip`.

Each mail is the same as this one but with different password and zip. In the data, we have the zip as base64. 

It seem that we need to recover all those zip to extract them with their corresponding passwords to recover the transfered file.
...
- Using Network Miner application, by this way we only need to open the `phreaky.pcap` with it, go to the message tab, right click on the attachment zip and select "open folder". Unfortunately Network Miner is a windows application, i tried to run it using "mono" but at "open folder" step it seem to crash. Maybe we can use wine, but i don't like wine.

Here i will show you using Network Miner, but on my Windows VM. 

I run my VM and download Network Miner which can been found bellow.

<a href="https://www.netresec.com/?page=NetworkMiner">Network Miner</a>
<a href="https://www.netresec.com/?download=NetworkMiner">Direct Download Link Network Miner</a>

Then i transfere the pcap on my VM and open it with Network Miner with `file > open`.
...
Next, as i need to extract all of them with their corresponding password, i come back to the mail tab, and write a list of the password in the correct order, and same with the zip, to create a quick bash script which will extract all of them.

Here is the script.

> Note : You need to have 7zip installed, give it execution right using `chmod +x` and runin using `bash script.sh`.
...
I transfere all the zip on my kali machine, and execute my script in the same location.
...
Then i list files to see that everything is ok.
...
Great, we unziped all the zip archive, but we end to 15 part of a pdf.

Now we need to find a way to merge them as one PDF. To do this we can use `cat` and the wildcard `*` to read them all at once and use `> phreaks_plan.pdf` to save the content into the `phreaks_plan.pdf` output. The wildcard will read them all in the chronological order because of the part numbers.
...
> Note : Ok! It seem so easy, and so oblivious, but guess what? Can you imagine that once i got all the flag part i tried to craft the full PDF by using `cat` against part 1, save to output, then `cat` against part 2, AND SAVE AT THE END OF THE PREVIOUS OUTPUT `>> phreaks_plan.pdf` i end up with a corrupted pdf!!!
...
We can see three GET request from `10.10.10.22` to `10.10.10.21`, two request seem identical which target the `/nBISC4YJKs7j4I` endpoint. And another GET request target the `/aQ4caZ.exe` endpoint, which seem to be an executable.
...
I removed a lot of the part, to get a better rendering in my writeup, but we can see that the function look like a malware as exemple, as seem to hint the description of the challenge.
...
EZRAT is a .NET malware written in C# for windows which had a lot of features (take a look to the github repo).

Finally, we can see something which look like a part of the flag : `Very_S3cr3t_S`

**Flag Part? : Very_S3cr3t_S**

Great! But what next? As i don't have enough information to know what to do with this, i come back to Wireshark, follow the TCP stream of the GET request on the executable by selecting it and using "CTRL+ALT+SHIFT+T", and i start to look the next streams using the arrow at the bottom right of the window.

<p align="center">
	<img src="assets/img/forensics/DataSiege/5-Next_Stream.jpg">
</p>

From there we found a lot of encoded strings, which look like base64. And into those, we found a powershell command with another base64 string.
...
Now let's analyse the founded domain `https://windowsliveupdater.com`. To do this i use curl with the `-I` parameter to only show website informations.
...
As we can see, the domain redirect to another domain `http://makelaris.com` repeath the process.
...
Error 302 Moved Permanently, look like the website redirect to the Location `https://makelaris.com`, let's repeat the process targeting HTTPS.
...
No redirection anymore, now let's use `curl` withtout parameter, to see the source of the main page.
...
And we get Rick Rolled!

Ok, now i guess that we need to decode all the base64 found in the request where we found the powershell encoded commmand. The problem? I'm not able to decode those!

So i take a look at the source code of EZRATClient on github, to understand how work the tool.

<a href="https://github.com/Exo-poulpe/EZRAT/blob/master/EZRATClient/Program.cs">EZRATClient - Program.cs Source Code</a>

From there, i discoverd two fonction to Encrypt AND Decrypt the stolen data!
...
And it convert to base64, this is why the content look like a base64. 
...
Great. So we know that the code contain the decryption function, which will help us to decrypt the data. But we need the key, and also, nothing said that the code isn't modified to encrypt data using another method.

So i continue to read the sources of the EZRATClient, and find this source code where i found the encrypt key.
...
Great. My next goal what to do the same process on the extracted executable from the capture.

And as the executable is in .NET, we can disassemble it. At the moment to solve the challenge i used DotPeek from JetBrains, on my windows machine to do the work.

<a href="https://www.jetbrains.com/decompiler/">JetBrains Decompiler - DotPeek</a>

But this time i will use AvaloniaILSpy, directly on my Kali Linux machine.
...
I start by copy pasting the Decrypt function to a new file, and add the needed library, and englobe the function into a `Public class Program`.
...
Then i need to declare what is `cipherText`, for this i use `Console.WriteLine` and `Console.ReadLine` to use user input.
...
So i come back to the original source code on github, and start to compare them.
...
Apparently, the `return cipherText;` and and `return "error"`, give me some error, also `cipherText` seem not taking in count at this location of the script, due to be outside the glob.

To fix this, i just delete them, and replace them with some `Console.WriteLine`. Also i notice that the last `catch (Exception ex)` is not at the correct position so i fix that too.

Now my script look like this.

```c#
// Custom EZRAT Decryptor
// Made by v0lk3n to solve DataSiege challenge from HTB Cyber Apocalypse CTF 2024
using System;
using System.IO;
using System.Security.Cryptography;
using System.Text;
...
I try to compile it, and it seem to compile it with success! I end up with an executable called `decryptor.exe`.
...
Let's try to run it using `mono` to look if everything seem to work as intended.
...
Great! Now that our decoder is ready, we need to retrieve all the encrypted data to decode. First i come back to wireshark to analyse few of the request which contain the data.

<p align="center">
	<img src="assets/img/forensics/DataSiege/9-Malware_Encrypt_Key.jpg">
</p>

We can see that all of those request are made on the port "1234", so i use `tshark` to dump all of those request in hexadecimal format to a file named `encrypted_data.txt`.

```bash
$ tshark -r capture.pcap -T fields -e data -Y "tcp.port == 1234" > encrypted_data.txt
```

Then i made a quick bash script which will read each line of the hexadecimal text file and decode them using `xxd`, and save the output to a file. 
...
Now i have a more readable file, but we need to take care of the beginning of each line which seem to contain the length of the line. Here is the beginning of the text file for example.
...
There is one string that i failed to decrypt (maybe it should be decoded to a file or there is two base64 in one line), which is this one.
...
There is also two base64 found in one line later, which is this one, that i needed to split.

```bash
24§uib3VErvtueXl08f8u4nfQ==24§uib3VErvtueXl08f8u4nfQ==
```

But as you can see both are the same. Now time for the decrypted result (i show only output for a better result).
...
We found the first flag part into the SSH `authorized_keys` file!
...
We found the second and last part of the flag into the user `svc01` Documents folder, in a `credentials.txt` file!
...
> Note : So yeah... In the beginning, into the `strings` result of the malware executable, the strings `Very_S3cr3t_S` was a rabbit hole to push you to think that you got the second and third part of the flag. So the first idea is to step back, like if you missed something. But as you see... WE SHOULD NOT! I haven't dig a lot to see where this strings are comming from. But if you build the original EZRAT, and made a strings on the exe, you will see that it look like the Author Name or something like that.
...
Using the command file to see which binary it is show that it's an ELF 64bit.
...
Running `strings` against it show some interesting things. (I removed a lot of content and replace with `...` to avoid showing the whole content)
...
At my first search, ive found this interesting blog post about it, but this is going too far, and in our case, we really can retrieve the flag by unpacking it using UPX, but bellow is the blog post for ressources.
...
Download it, give it the execution right with `chmod +x` and run it to see how to use the tool.
...
First we start the docker, and take a loot of the Web App.

<p align="center">
	<img src="assets/img/web/FlagCommand/1-WebApp.jpg">
</p>

> Note : This game remembered me the game eXit from Mr.Robot series, few years ago, you was able to play the game online. But unfotunatly the game is no longer up, and web archive capture seem not allow you to play on this one... But some people made great port of this on github if you search for it.  For those who didnt see Mr.Robot show and are planning to see it, SPOILER WARNING ! Also <a href="https://www.youtube.com/watch?v=g_Miz2ZqSI4">here is a video about the eXit game scene</a> on Mr.Robot. 
 
Oh nice! We find a text game. Let's try to start the game to see what we can do.

<p align="center">
	<img src="assets/img/web/FlagCommand/2-Start_the_game.jpg">
</p>

Ok, nothing that will help apparently, let's try to run a system command to see if it work.

Not working, but we got a hint! 

<p align="center">
	<img src="assets/img/web/FlagCommand/3-Pick_from_options.jpg">
</p>

So we need to "pick from options", let's look at code source of the page.
...
We found some interesting page, `command.js`, `main.js` and `game.js` all of them are into `/static/terminal/js/` path.

Taking a look at `command.js` with didnt find anything usefull excepted which message we get once we win or we lose the game.
...
Finally, let's take a look at `main.js`, and there, a lot of things are interesting. Lets take all of this one by one and digg on it.
...
Let's cut this. Here you can see that a `secret` command seem to exist, and if triggered it will fetch `/api/monitor` with a POST request using as data the secret command used.
...
Then we seen that if the response containt `Game Over` we lost, but if it containt `HTB{` we won! Seem that we found how to retrieve the flag.
...
Come back to the game, reset the game with the `restart` command if needed, the `start` the game, and submit the secret command.

<p align="center">
	<img src="assets/img/web/FlagCommand/5-flag.jpg">
</p>

And we got the flag! But! Here is a problem, it seem hard to copy the flag, and as the flag say, dev command are the best, so use the dev tools to copy it.
...
We got a login page, let's try to guess basic combinaison such as `admin:admin`.
...
I go back to the proxy tab of burp suite, and under action > save item, and i save the request to file called `sqlrequest.txt`.

Then i fire SQLMap against the request file with `-r sqlrequest.txt` and no parameter.
...
- set the dbms to mysql and test injection for this specify dmbs
- to dump all databases, tables and entry.
- batch is used to automate my answer as i used the default one each time
...
Great! We got the username `admin` with the password hash `$2b$12$OF1QqLVkMFUwJrl1J1YG9u6FdAQZa6ByxFt/CkS/2HW8GA563yiv.` from the database `korp_terminal`, on the tables `users`.

My next step was to identify that hash type, it look like bcrypt but to be sure we can have a look to the hashcat list examples hashes, that you can find bellow.
...
Great! We sucesfully cracked it, so the username `admin` password is `password123`.
...
> Note : With my code review skills, i dont think that my solution is the most complete. So i encourage you to read another writeup after this one.

As i'm not the best with code review, i start the docker and look how the Web App work.

<p align="center">
	<img src="assets/img/web/TimeKORP/1-WebApp.jpg">
</p>

As we can see, we can look the time and date. We can play with this input by modifying the `?format=%H:%M:%S` and reflect the output.
...
Taking a look at `TimeController.php` we see that.
...
We can see that the command is constructed by this way `$this->command = "date '+" . $format . "' 2>&1";`, and then pass it to `exec()` here `$time = exec($this->command);`. So we may find a way to escape this to make a command injection.

As i said, i'm not strong enough in code review, maybe someone can figure how directly, but not me. So i've asked ChatGPT some more information to explain me those command.
...
Thats great for us! Because as the command pass to exec() function, we should be able to escape the date command using a quote, to close the "date" input, and add `;` to add our additional system command and close it again with a quote.
...
Great it seem to work! But apparently, it retrieve only one output instead of each line, but it should be enough to recover the flag based on the content of the fake flag which had it as one line. So now, let's try to send the system command `ls` using `';ls'` as payload. 
...
Doing some google search about those, to look for potential vulnerability targetting those version and we find a relevant vulnerability.
...
Then i've found a cool blog explaining Server Side Template Injection (SSTI) exploitation against Velocity 1.7, that you can find bellow.
...
As said the description, its a translator web app, we are able to send text and translate it from english to "voxalith" language. Which seem unreadable. But we can use Burp Suite to read the output, or the dev tools.

I prefere to use burp suite for web stuff, so i start Burp use the dedicated chormium browser and intercept the request by sending "Hello There", then i send to repeater and send the request.
...
If the output is the whole payload, it shouldnt be vulnerable, else if the output is "2", it mean our code is executed because our payload say "1+1".

So i modify my previous command to with the payload on Burp, select it and use "CTRL+U" to URL encode my payload, finally i send the request.
...
Then i URL encode it with "CTRL+U" and send the request.
...
Instead of reading source code (because i'm really bad for it), i continue my google search and find a thread on GitHub talking about this, you can find it bellow.
...
So i took the payload which will run the command `ls -al`, put it in Burp and URL encode it using "CTRL+U", then send my request.

<p align="center">
	<img src="assets/img/web/LabyrinthLinguist/5-RCE_Success.jpg">
</p>

And it work! We listed successfully the files and directory in our current path. Now lets go back to the sources code to see where is located our flag.
...
But we cant read the flag, why? To know this, we need to go back on source again.

Reading `entry_point.sh` file, we can see that the flag is renamed to a random strings while starting the docker.
...
Now i use the command `cat ../flag26df64a005.txt` and of course use `+` instead of space to get it URL encoded `cat+../flag26df64a005.txt`, and submit my request.
